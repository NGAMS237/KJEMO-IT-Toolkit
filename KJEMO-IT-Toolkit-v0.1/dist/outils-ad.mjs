...
export const outilCheminsProfils = {
  ...
  risk: 'caution',
  summary: 'Vérifie partage, dossiers et permissions, sauvegarde les anciennes valeurs — et n\u2019écrit rien si cette sauvegarde échoue — puis pose HomeDirectory, HomeDrive et ProfilePath de façon idempotente.',
  fields: [
    champDomaine(),
  ...
  generate(v) {
    ...
    const corps = [
      ...
      "# La sauvegarde n'est pas une precaution decorative : c'est elle, et elle",
      "# seule, qui rend ce geste reversible. Si elle echoue, RIEN n'est ecrit :",
      '# ni dossier cree, ni permission posee, ni attribut d\'annuaire modifie.',
      "# L'indicateur ci-dessous est la seule porte vers le bloc Appliquer.",
      '$sauvegardeOk = $false',
      '$sauvegarde = Join-Path (Join-Path $env:USERPROFILE \'Desktop\') ("kjemo-ad-chemins-avant-" + (Get-Date -Format \'yyyyMMdd-HHmmss\') + ".csv")',
      'if (@($ancien).Count -eq 0) {',
      '  # Aucun compte dans le perimetre : il n\'y a rien a sauvegarder, et rien a',
      '  # modifier non plus. La porte reste ouverte, mais elle ne mene nulle part.',
      '  $sauvegardeOk = $true',
      "  [void](Add-KjemoResultat -Categorie 'Sauvegarde' -Controle 'Anciennes valeurs' -Etat 'IGNORE' -Valeur 'aucun compte a sauvegarder' -Commentaire 'Aucun compte dans cette OU : il n''y a ni valeur a conserver, ni modification a faire.')",
      '} else {',
      '  try {',
      '    $ancien | Export-Csv -LiteralPath $sauvegarde -NoTypeInformation -Encoding UTF8 -ErrorAction Stop',
      '    $sauvegardeOk = $true',
      ...
      '  } catch {',
      '    $sauvegardeOk = $false',
      "    [void](Add-KjemoResultat -Categorie 'Sauvegarde' -Controle 'Anciennes valeurs' -Etat 'PROBLEME' -Valeur $_.Exception.Message -Commentaire 'La sauvegarde a echoue : AUCUNE modification ne sera executee. Verifie l''acces en ecriture au Bureau, puis relance.')",
      '  }',
      '}',
      'if (-not $sauvegardeOk) {',
      "  [void](Add-KjemoResultat -Categorie 'Action' -Controle 'Modifications' -Etat 'IGNORE' -Valeur 'bloquees par l''echec de la sauvegarde' -Commentaire 'Sans copie des anciennes valeurs, le geste ne serait pas reversible : le script s''arrete avant d''ecrire quoi que ce soit.')",
      '}',
      "",
      '# --- 4. Application, uniquement en mode Appliquer ET sauvegarde reussie -----',
      "if ($Mode -eq 'Appliquer' -and $moduleOk -and $sauvegardeOk) {",
      ...
      '} else {',
      "  Write-Host ''",
      "  if ($Mode -eq 'Appliquer' -and -not $sauvegardeOk) {",
      "    Write-Host '--- Aucune modification : la sauvegarde a echoue ---'",
      "    Write-Host 'Le mode Appliquer a ete demande, mais la copie des anciennes valeurs'",
      "    Write-Host 'n''a pas abouti. Aucun dossier n''a ete cree, aucune permission posee,'",
      "    Write-Host 'aucun attribut d''annuaire modifie. Corrige la sauvegarde, puis relance.'",
      '  } else {',
      "    Write-Host '--- Simulation (-WhatIf) ---'",
      '    foreach ($cible in $aModifier) {',
  ...
  commonErrors: ERREURS_AD.concat([
    ...
    {
      message: 'Le rapport annonce « Modifications : bloquées par l\u2019échec de la sauvegarde »',
      cause: 'L\u2019export CSV des anciennes valeurs n\u2019a pas pu être écrit : Bureau en lecture seule, profil itinérant non chargé, disque plein, ou antivirus qui verrouille le fichier.',
      fix: 'C\u2019est volontaire, et rien n\u2019a été modifié. Corriger l\u2019accès en écriture au Bureau de la session, puis relancer : la sauvegarde conditionne toute écriture.',
      command: 'Get-Item -LiteralPath ([Environment]::GetFolderPath(\'Desktop\')) | Format-List FullName,Attributes',
    },
  ]),
  reversible: true,
  verifyAfter: [
    'Le fichier CSV des anciennes valeurs est écrit sur le Bureau avant toute modification — et si cette écriture échoue, le script n\u2019en fait aucune : ni dossier, ni permission, ni attribut.',
  ...
  checks: [
    'La sauvegarde des anciennes valeurs conditionne tout le reste : si elle échoue, rien n\u2019est modifié.',
...
export const outilCanalSecurise = {
  ...
  generate(v) {
    ...
    const corps = [
      ...
      '    $srvOk = $true',
      '    # Un enregistrement SRV renvoie un nom absolu, termine par un point.',
      "    # Passe tel quel a -Server, ce point fait echouer la resolution.",
      "    $cible = $reponses[0].NameTarget.TrimEnd('.')",
      '  }',
      '} catch { }',
      'if ($srvOk) {',
      ...
      "  if ($cible) { $cible = $cible.TrimStart('\\\\').TrimEnd('.') }",
      '}',
      '',
      '# Sans controleur identifie, le canal ne peut etre ni teste utilement, ni',
      "# repare : -Server designe UN controleur, pas un domaine. On le dit ici,",
      '# plutot que de laisser la commande echouer avec un message obscur.',
      'if (-not $cible) {',
      "  [void](Add-KjemoResultat -Categorie 'Prealables' -Controle 'Controleur de domaine retenu' -Etat 'PROBLEME' -Valeur 'aucun' -Commentaire 'Ni les enregistrements SRV, ni le systeme n''ont designe de controleur. Corrige le DNS ou la decouverte du controleur : aucune reparation ne sera tentee.')",
      '} else {',
      "  [void](Add-KjemoResultat -Categorie 'Prealables' -Controle 'Controleur de domaine retenu' -Etat 'OK' -Valeur $cible -Commentaire 'C''est ce controleur precis qui sera interroge, et non le nom general du domaine.')",
      '}',
      "",
      ...
      '$canalOk = $null',
      'if ($estMembre) {',
      '  # -Server attend UN controleur de domaine, pas le nom du domaine :',
      '  # https://learn.microsoft.com/powershell/module/microsoft.powershell.management/test-computersecurechannel',
      "  # On passe donc le controleur retenu a l'etape 2. S'il manque, on laisse",
      '  # la machine choisir elle-meme plutot que de lui donner une valeur fausse.',
      '  try {',
      '    if ($cible) {',
      '      $canalOk = Test-ComputerSecureChannel -Server $cible -ErrorAction Stop',
      '    } else {',
      '      $canalOk = Test-ComputerSecureChannel -ErrorAction Stop',
      '    }',
      '  } catch {',
      ...
      "if ($Mode -eq 'Appliquer' -and $estMembre -and $canalOk -eq $false -and -not $cible) {",
      "  [void](Add-KjemoResultat -Categorie 'Action' -Controle 'Reparation du canal' -Etat 'PROBLEME' -Valeur 'impossible : aucun controleur de domaine identifie' -Commentaire 'La reparation doit viser un controleur precis. Corrige d''abord le DNS du domaine, ou la decouverte du controleur, puis relance. Aucune reparation n''a ete tentee.')",
      "} elseif ($Mode -eq 'Appliquer' -and $estMembre -and $canalOk -eq $false) {",
      "  Write-Host ''",
      ...
      '  if ($identifiants) {',
      '    try {',
      '      $resultat = Test-ComputerSecureChannel -Repair -Credential $identifiants -Server $cible -ErrorAction Stop',
      "      $etatR = 'PROBLEME'",
      ...
      '  $apres = $null',
      '  try { $apres = Test-ComputerSecureChannel -Server $cible -ErrorAction SilentlyContinue } catch { }',
      "  $etatV = 'PROBLEME'",
      ...
      "  Write-Host 'executerait, en visant le controleur retenu et non le domaine :'",
      '  Write-Host "  Test-ComputerSecureChannel -Repair -Credential (Get-Credential) -Server $cible"',
      '}',
      blocModeDiagnostic(),
    ];
  ...
  commonErrors: ERREURS_AD.concat([
    ...
    {
      message: 'Test-ComputerSecureChannel : le paramètre -Server ne trouve pas le serveur',
      cause: 'Le nom du domaine a été passé à -Server, qui attend un contrôleur de domaine précis, ou le nom fourni se termine par un point hérité d’un enregistrement SRV.',
      fix: 'Le script résout lui-même un contrôleur par l’enregistrement SRV, retire le point final, et n’agit que s’il en tient un.',
      command: 'Resolve-DnsName -Name \'_ldap._tcp.dc._msdcs.<domaine>\' -Type SRV | Select-Object NameTarget,Port',
    },
    {
  ...
  verifyAfter: [
    ...
    'Le rapport nomme le contrôleur retenu : diagnostic, réparation et vérification portent tous sur celui-là.',
  ...
  rollback: {
    ...
    command: '# Il n\'y a rien a defaire. Si le canal est de nouveau rompu, on recommence le\n# diagnostic : DNS, heure, objet ordinateur, puis canal.\nw32tm /resync\n\n# -Server attend UN controleur de domaine, pas le nom du domaine. On le\n# retrouve par l\'enregistrement SRV, dont on retire le point final.\n$dc = (Resolve-DnsName -Name \'_ldap._tcp.dc._msdcs.<domaine>\' -Type SRV | Where-Object { $_.QueryType -eq \'SRV\' })[0].NameTarget.TrimEnd(\'.\')\nTest-ComputerSecureChannel -Server $dc -Verbose',
  ...
  checks: [
    'Le script vise un contrôleur précis, pas le nom du domaine : sans contrôleur identifié, il refuse de réparer.',

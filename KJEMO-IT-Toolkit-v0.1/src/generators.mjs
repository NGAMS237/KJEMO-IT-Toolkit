 ...
 * -------------------------------------------
 * Module ES exportant :
 *   - Fonctions d'échappement : escapePowerShellSingleQuoted, escapePowerShellDoubleQuoted, escapeLdapRdn
 *   - Validateurs stricts : validateDomain, validateIPv4, validateSamAccountName,
 *       validateGroupName, validateShareName, validateIntegerStrict, validateFloatStrict,
 *       validateOuName, validatePath
 *   - Utilitaires : domainToDn, normalizeScript, textToCode
 *   - Tableau tools (8 assistants)
 *
 * Importé par dist/app.js (site statique GitHub Pages) ET par les tests Node.js.
 ...
 * En PS double-quoted, le backtick, $ et " sont des caractères spéciaux.
 * @param {unknown} v
 * @returns {string}
 */
...
export function escapeLdapRdn(v) {
  let s = String(v ?? '');
  // 1. Backslash en premier (sinon les backslashes d'échappement seraient eux-mêmes échappés)
  s = s.replace(/\\/g, '\\\\');
  // 2. Caractères spéciaux RFC 4514
  s = s.replace(/[,+="<>;]/g, (c) => '\\' + c);
  // 3. Dièse en début de valeur
  if (s.startsWith('#')) s = '\\#' + s.slice(1);
  // 4. Espaces en début et en fin
...
/**
 * Valide un nom de domaine (RFC 1123).
 * @param {unknown} v
...
export function validateDomain(v) {
  ...
  for (const label of labels) {
    ...
    if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$|^[a-zA-Z0-9]$/.test(label)) {
      return { ok: false, value: s, error: `Le composant « ${label} » contient des caractères non autorisés (lettres, chiffres et tirets seulement).` };
    }
  }
...
/**
 * Valide un SamAccountName Active Directory.
 * @param {unknown} v
 * @returns {{ ok: boolean, value: string, error: string|null }}
 */
...
/**
 * Valide un nom de groupe Active Directory.
 * @param {unknown} v
 * @returns {{ ok: boolean, value: string, error: string|null }}
 */
export function validateGroupName(v) {
  ...
  if (/[\x00-\x1f\x7f]/.test(s)) {
    return { ok: false, value: s, error: 'Le nom du groupe contient un caractère de contrôle non autorisé.' };
  }
  return { ok: true, value: s, error: null };
}

/**
 * Valide un nom de partage SMB Windows.
 * @param {unknown} v
 * @returns {{ ok: boolean, value: string, error: string|null }}
 */
...
/**
 * Valide un entier strict : refuse « 12abc », les chaînes vides, et les valeurs hors plage.
 * @param {unknown} v
 * @param {number|undefined} min
 * @param {number|undefined} max
 * @param {string} label  Libellé du champ pour le message d'erreur.
...
export function validateIntegerStrict(v, min, max, label) {
  ...
  if (!/^-?\d+$/.test(s)) {
    return { ok: false, value: null, error: `${lbl} doit être un entier entier (exemple : 12). Valeur saisie : « ${s} ».` };
  }
  const n = Number(s);
...
/**
 * Valide un nombre décimal strict : refuse « 0.5abc », les chaînes vides, et les valeurs hors plage.
 * @param {unknown} v
 * @param {number|undefined} min
 * @param {number|undefined} max
 * @param {string} label
...
/**
 * Valide un nom d'OU Active Directory.
 * @param {unknown} v
 * @returns {{ ok: boolean, value: string, error: string|null }}
 */
...
/**
 * Valide un chemin de fichier local.
 * @param {unknown} v
 * @returns {{ ok: boolean, value: string, error: string|null }}
 */
...
/**
 * Convertit un nom de domaine en Distinguished Name LDAP.
 * Chaque composant est échappé selon les règles RDN.
 * @param {string} domain
 * @returns {string}
 */
 ...
 * Utilisé dans la source JS pour éviter les backticks littéraux dans les template literals.
 * @param {string} text
 * @returns {string}
 */
...
// ---------------------------------------------------------------------------
// Raccourci interne : le plus courant
// ---------------------------------------------------------------------------
...
export const tools = [
  // ── 1. IP statique ──────────────────────────────────────────────────────
  {
    ...
    validate(v) {
      const errors = {};
      if (!String(v.adapter ?? '').trim()) errors.adapter = 'Le nom de la carte ne peut pas être vide.';
    ...
    generate(v) {
      ...
      const ip      = escPs(v.ip);
      const prefix  = validateIntegerStrict(v.prefix, 1, 32, 'Préfixe').value ?? 24;
  ...
  // ── 2. Créer une OU ─────────────────────────────────────────────────────
  {
    ...
    generate(v) {
      ...
      // OU name: LDAP-escaped then PS-escaped (for embedding in PS single-quoted strings containing DN)
      const ouPs    = escPs(v.ou);
      const ouLdap  = escapeLdapRdn(v.ou);
      const pathDn  = v.parent ? `OU=${escapeLdapRdn(v.parent)},${dn}` : dn;
      const pathPs  = v.parent ? `OU=${escPs(v.parent)},${dn}` : dn;
      const fullPs  = `OU=${ouPs},${pathPs}`;
  ...
  // ── 3. Créer un utilisateur AD ───────────────────────────────────────────
  {
    ...
    validate(v) {
      ...
      if (!String(v.firstName ?? '').trim()) errors.firstName = 'Le pr\u00e9nom ne peut pas \u00eatre vide.';
      if (!String(v.lastName  ?? '').trim()) errors.lastName  = 'Le nom ne peut pas \u00eatre vide.';
      return errors;
    },
    generate(v) {
      const dn        = `OU=${escPs(v.ou)},${domainToDn(v.domain)}`;
  ...
  // ── 4. Dossier partagé ───────────────────────────────────────────────────
  {
    ...
    generate(v) {
      ...
      const access = ['Change', 'Read', 'Full'].includes(v.access) ? v.access : 'Change';
      return (
  ...
  // NOTE : le champ « dns » de la version originale n'était jamais utilisé
  // dans generate(). Il est retiré ici conformément aux corrections LOT 0.
  {
    id: 'second-dc',
    icon: '\u21c4',
    category: 'Active Directory',
    ...
    validate(v) {
      const errors = {};
      const dom = validateDomain(v.domain); if (!dom.ok) errors.domain = dom.error;
      if (!String(v.source ?? '').trim()) errors.source = 'Le contr\u00f4leur source ne peut pas \u00eatre vide.';
      return errors;
    },
    generate(v) {
      const domain = escPs(v.domain);
  ...
  // ── 7. Politique de mots de passe ────────────────────────────────────────
  {
    ...
    generate(v) {
      const domain   = escPs(v.domain);
      // validateIntegerStrict returns null only when value is invalid — callers must have
      // already validated; we use a safe fallback that preserves 0 (valid lockout threshold).
      const length   = validateIntegerStrict(v.length,  1, 256, '').value ?? 12;
      const lockoutR = validateIntegerStrict(v.lockout, 0, 999, '');
      const lockout  = lockoutR.ok ? lockoutR.value : 5;   // 0 is valid — preserve it
      const minutes  = validateIntegerStrict(v.minutes, 1, 99999, '').value ?? 30;
      return (
        `Import-Module ActiveDirectory\n\n` +
...
// ── 8. Analyse de disque ─────────────────────────────────────────────────────
// Outil complexe : factory séparée pour la lisibilité.
function createDiskScanTool() {
  return {
    ...
    generate(v) {
      const top     = validateIntegerStrict(v.top,     5, 200,    '').value ?? 50;
      const minSize = validateFloatStrict(  v.minSize, 0, 100000, '').value ?? 0.5;
      ...
      const lines = [
        ...
        '    })',
        // BUG LOT 0 CORRIGÉ : n\u2019est → n''est (U+2019 → deux apostrophes droites)
